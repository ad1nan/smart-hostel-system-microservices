...
function App() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const endpointList = [
        { key: "rooms", endpoint: "/rooms", setter: setRooms },
        { key: "devices", endpoint: "/devices", setter: setDevices },
        { key: "alerts", endpoint: "/alerts", setter: setAlerts },
        { key: "heatmap", endpoint: "/analytics/heatmap", setter: setRoomAnalytics },
        { key: "device analytics", endpoint: "/analytics/devices", setter: setDeviceAnalytics },
        { key: "time series", endpoint: "/analytics/timeseries", setter: setTimeSeriesAnalytics }
      ];
      const results = await Promise.allSettled(endpointList.map((entry) => API.get(entry.endpoint)));
      const failures = [];
      results.forEach((result, idx) => {
        const entry = endpointList[idx];
        if (result.status === "fulfilled") {
          entry.setter(Array.isArray(result.value.data) ? result.value.data : []);
          return;
        }
      ...
      setDataIssues((prev) => {
        const baseIssues = prev.filter((issue) => !issue.startsWith("Realtime"));
        if (!failures.length) return baseIssues;
        return [...baseIssues, `Realtime data missing: ${failures.join(", ")}`];
      });
    } catch (err) {
  ...
  const fetchAdvancedAnalytics = useCallback(async () => {
    ...
    try {
      const endpointList = [
        { key: "forecast", request: API.get("/analytics/forecast", { params: { method: "linear" } }), setter: setForecastData },
        { key: "room costs", request: API.get("/analytics/room-costs", { params: { ratePerKwh, windowHours: 24 } }), setter: setRoomCosts },
        { key: "peak hours", request: API.get("/analytics/peak-hours"), setter: setPeakHours },
        { key: "daily reports", request: API.get("/analytics/reports/daily", { params: { limit: 7 } }), setter: setDailyReports },
        { key: "weekly reports", request: API.get("/analytics/reports/weekly", { params: { limit: 6 } }), setter: setWeeklyReports }
      ];
      const results = await Promise.allSettled(endpointList.map((entry) => entry.request));
      const failures = [];
      results.forEach((result, idx) => {
        const entry = endpointList[idx];
        if (result.status === "fulfilled") {
      ...
      setDataIssues((prev) => {
        const baseIssues = prev.filter((issue) => !issue.startsWith("Analytics"));
        if (!failures.length) return baseIssues;
        return [...baseIssues, `Analytics data missing: ${failures.join(", ")}`];
      });
    } catch (err) {
  ...
  return (
    <div className={`app ${darkMode ? "theme-dark" : "theme-light"} ${compactView ? "compact" : "expanded"}`}>
      ...
      {dataIssues.length > 0 && (
        <div className="data-issues-banner">
          {dataIssues[dataIssues.length - 1]}
        </div>
      )}
